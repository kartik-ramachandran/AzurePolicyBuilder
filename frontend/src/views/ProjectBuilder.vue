<template>
  <div class="project-builder min-h-[calc(100vh-7rem)] text-slate-950">
    <div :class="['relative isolate min-h-[calc(100vh-7rem)] overflow-hidden', hasAnalyzed ? 'review-shell' : 'welcome-surface']">
      ...
      <main v-if="!hasAnalyzed" class="welcome-stage">
        <section class="welcome-copy">
          ...
          <div class="mt-4 flex flex-col items-start gap-2">
            <button class="text-sm font-bold text-blue-700 underline decoration-sky-300 underline-offset-4 hover:text-blue-800" @click="startManually">
              No contract? Compose the package manually →
            </button>
            <button class="text-sm font-bold text-blue-700 underline decoration-sky-300 underline-offset-4 hover:text-blue-800" @click="openImportModal">
              Import an existing package — ZIP, folder, GitHub, or live Azure APIM →
            </button>
          </div>
        ...
        <section class="review-content overflow-y-auto">
          <div v-if="builderMode === 'review'" class="review-page">
            ...
            <div class="source-ribbon">
              ...
              <button @click="triggerFilePicker">Add new contract</button>
            </div>
      ...
      <div v-if="showImportModal" class="fixed inset-0 z-[60] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm">
        <div class="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-hidden rounded-[2rem] border border-sky-100 bg-white shadow-2xl shadow-slate-900/20">
          <header class="border-b border-sky-100 bg-gradient-to-r from-white to-sky-50 px-6 py-5">
            <p class="text-xs font-black uppercase tracking-[0.35em] text-blue-500">Import</p>
            <h2 class="mt-1 text-3xl font-black text-slate-950">Import existing package</h2>
          </header>
            ...
            <div class="mb-5 flex flex-wrap gap-2">
              <button
                ...
                :class="[
                  ...
                  importSource === source.id ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'bg-sky-50 text-blue-700 hover:bg-sky-100'
                ]"
                @click="selectImportSource(source.id)"
              >
                {{ source.label }}
              </button>
            </div>

            <div v-if="importSource === 'zip'" class="grid gap-4">
              <p class="text-sm text-slate-600">Upload a package ZIP previously generated by this tool (or any APIOps-style archive).</p>
              <input
                type="file"
                accept=".zip,application/zip"
                class="w-full rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm"
                @change="onImportZipSelected"
              />
            </div>

            <div v-else-if="importSource === 'folder'" class="grid gap-4">
              <p class="text-sm text-slate-600">Point at a folder on this machine containing an APIM package (the folder with <code class="rounded bg-slate-100 px-1">apis/</code>, <code class="rounded bg-slate-100 px-1">policy.xml</code>, …).</p>
              <input
                v-model="importFolderPath"
                type="text"
                placeholder="C:\path\to\apim-package"
                class="w-full rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm font-mono"
              />
            </div>

            <div v-else-if="importSource === 'github'" class="grid gap-4">
              <label class="space-y-2">
                <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Repository URL</span>
                <input v-model="importGitHubForm.repoUrl" type="text" placeholder="https://github.com/owner/repo" class="w-full rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm" />
              </label>
              <div class="grid gap-4 sm:grid-cols-2">
                <label class="space-y-2">
                  <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Branch (optional)</span>
                  <input v-model="importGitHubForm.branch" type="text" placeholder="main" class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm" />
                </label>
                <label class="space-y-2">
                  <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Subfolder (optional)</span>
                  <input v-model="importGitHubForm.path" type="text" placeholder="infra/apim" class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm" />
                </label>
              </div>
              <label class="space-y-2">
                <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Personal Access Token (private repos)</span>
                <input v-model="importGitHubForm.pat" type="password" placeholder="ghp_… (not stored)" class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm" />
                <span class="block text-xs text-slate-500">Used only for this request; never saved.</span>
              </label>
            </div>

            <div v-else class="grid gap-4">
              <p class="text-sm text-slate-600">
                Pulls from a live instance using your Azure sign-in — run <code class="rounded bg-slate-100 px-1">az login</code> on the machine hosting the backend first.
              </p>
              <div class="grid gap-4 sm:grid-cols-3">
                <label class="space-y-2 sm:col-span-3">
                  <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Subscription ID</span>
                  <input v-model="importAzureForm.subscriptionId" type="text" placeholder="00000000-0000-0000-0000-000000000000" class="w-full rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3 text-sm font-mono" />
                </label>
                <label class="space-y-2 sm:col-span-2">
                  <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Resource Group</span>
                  <input v-model="importAzureForm.resourceGroup" type="text" placeholder="rg-apim-prod" class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm" />
                </label>
                <label class="space-y-2">
                  <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">APIM Name</span>
                  <input v-model="importAzureForm.serviceName" type="text" placeholder="my-apim" class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm" />
                </label>
              </div>

              <button
                class="rounded-2xl border border-sky-200 bg-white px-5 py-3 text-sm font-black text-blue-700 hover:bg-sky-50 disabled:opacity-50"
                :disabled="importBusy"
                @click="loadAzureInventory"
              >
                {{ importBusy && !azureInventory ? 'Connecting…' : 'Load inventory' }}
              </button>

              <div v-if="azureInventory" class="rounded-2xl border border-sky-100 bg-sky-50 p-4">
                <p class="mb-2 text-sm font-black text-slate-900">
                  {{ azureInventory.serviceName }} — select APIs to import
                </p>
                <div class="max-h-48 space-y-1 overflow-y-auto">
                  <label v-for="api in azureInventory.apis" :key="api.name" class="flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm">
                    <input v-model="api.selected" type="checkbox" class="h-4 w-4 rounded border-sky-300 text-blue-600" />
                    <span class="font-bold text-slate-800">{{ api.displayName }}</span>
                    <span class="text-xs text-slate-500">{{ api.path || '/' }}</span>
                  </label>
                </div>
                <p class="mt-3 text-xs font-semibold text-slate-500">
                  Also imports: {{ azureInventory.productCount }} products, {{ azureInventory.namedValueCount }} named values,
                  {{ azureInventory.policyFragmentCount }} fragments, {{ azureInventory.backendCount }} backends.
                  Secret named values come back as placeholders.
                </p>
              </div>
            </div>

            <p v-if="importError" class="mt-4 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{{ importError }}</p>
          </div>

          <footer class="border-t border-sky-100 bg-white px-6 py-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
              <button class="rounded-2xl border border-slate-200 bg-white px-5 py-3 font-black text-slate-700 hover:bg-slate-50" @click="showImportModal = false">
                Cancel
              </button>
              <button
                class="rounded-2xl bg-blue-600 px-6 py-3 font-black text-white shadow-lg shadow-blue-200 hover:bg-blue-700 disabled:opacity-50"
                :disabled="importBusy || (importSource === 'azure' && !azureInventory)"
                @click="runImport"
              >
        ...
        <div class="max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-hidden rounded-[2rem] border border-sky-100 bg-white shadow-2xl shadow-slate-900/20">
          ...
          <div class="max-h-[calc(100vh-15rem)] overflow-y-auto px-6 py-5">
            ...
            <div v-else-if="artifactModalKind === 'product'" class="grid gap-4">
              <label v-if="libraryProducts.length" class="space-y-2">
                <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Start from library</span>
                <select
                  v-model="artifactStartFrom"
                  class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm text-slate-800 outline-none ring-sky-300/30 focus:ring-4"
                  @change="applyProductFromLibrary(artifactStartFrom)"
                >
                  <option value="">— Blank product —</option>
                  <option v-for="item in libraryProducts" :key="item.name" :value="item.name">{{ item.displayName || item.name }}</option>
                </select>
              </label>
              <label class="space-y-2">
              ...
              <label class="flex h-12 items-center gap-3 rounded-2xl border border-sky-100 bg-sky-50 px-4 text-sm font-black text-slate-700">
                <input v-model="saveToLibrary" type="checkbox" class="h-4 w-4 rounded border-sky-300 text-blue-600 focus:ring-blue-500" />
                Also save to reusable library
              </label>
            </div>

            <div v-else-if="artifactModalKind === 'namedValue'" class="grid gap-4">
              <label v-if="libraryNamedValues.length" class="space-y-2">
                <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Start from library</span>
                <select
                  v-model="artifactStartFrom"
                  class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm text-slate-800 outline-none ring-sky-300/30 focus:ring-4"
                  @change="applyNamedValueFromLibrary(artifactStartFrom)"
                >
                  <option value="">— Blank named value —</option>
                  <option v-for="item in libraryNamedValues" :key="item.name" :value="item.name">{{ item.displayName || item.name }}</option>
                </select>
              </label>
              <label class="space-y-2">
                <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Name</span>
            ...
            <div v-else class="grid gap-4">
              <label v-if="libraryFragments.length || policyTemplates.length" class="space-y-2">
                <span class="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Start from</span>
                <select
                  v-model="artifactStartFrom"
                  class="w-full rounded-2xl border border-sky-100 bg-white px-4 py-3 text-sm text-slate-800 outline-none ring-sky-300/30 focus:ring-4"
                  @change="applyFragmentStart(artifactStartFrom)"
                >
                  <option value="">— Blank fragment —</option>
                  <optgroup v-if="libraryFragments.length" label="My fragments (Fragment Manager)">
                    <option v-for="item in libraryFragments" :key="`fragment:${item.id}`" :value="`fragment:${item.id}`">{{ item.name }}</option>
                  </optgroup>
                  <optgroup v-if="policyTemplates.length" label="Policy templates (wrapped as fragment)">
                    <option v-for="item in policyTemplates" :key="`template:${item.id}`" :value="`template:${item.id}`">{{ item.name }}</option>
                  </optgroup>
                </select>
              </label>
              <label class="space-y-2">
              ...
              <label class="flex h-12 items-center gap-3 rounded-2xl border border-sky-100 bg-sky-50 px-4 text-sm font-black text-slate-700">
                <input v-model="saveToLibrary" type="checkbox" class="h-4 w-4 rounded border-sky-300 text-blue-600 focus:ring-blue-500" />
                Also save to Fragment Manager library
              </label>
            </div>
          </div>
...
import { projectService, importService, libraryService, fragmentService, templateService } from '@/services/api'
...
const importGitHubForm = reactive({ repoUrl: '', branch: '', path: '', pat: '' })
const importAzureForm = reactive({ subscriptionId: '', resourceGroup: '', serviceName: '' })
const azureInventory = ref<any | null>(null)
const globalPolicy = ref('')

const libraryProducts = ref<any[]>([])
const libraryNamedValues = ref<any[]>([])
const libraryFragments = ref<any[]>([])
const policyTemplates = ref<any[]>([])
const saveToLibrary = ref(false)
const artifactStartFrom = ref('')
...
function selectImportSource(source: ImportSource) {
...
function onImportZipSelected(event: Event) {
  const input = event.target as HTMLInputElement
  importZipFile.value = input.files?.[0] ?? null
}

async function loadAzureInventory() {
  importBusy.value = true
  importError.value = ''
  azureInventory.value = null
  try {
    const inventory = await importService.azureInventory({ ...importAzureForm })
    azureInventory.value = {
      ...inventory,
      apis: inventory.apis.map(api => ({ ...api, selected: true }))
    }
  } catch (e: any) {
    importError.value = e.response?.data?.error || e.message || 'Failed to reach Azure.'
  } finally {
    importBusy.value = false
  }
}
...
async function runImport() {
  ...
  try {
    let imported: any
    if (importSource.value === 'zip') {
      if (!importZipFile.value) throw new Error('Choose a ZIP file first.')
      imported = await importService.importZip(importZipFile.value)
    } else if (importSource.value === 'folder') {
      if (!importFolderPath.value.trim()) throw new Error('Enter a folder path.')
      imported = await importService.importFolder(importFolderPath.value.trim())
    } else if (importSource.value === 'github') {
      if (!importGitHubForm.repoUrl.trim()) throw new Error('Enter a repository URL.')
      imported = await importService.importGitHub({
        repoUrl: importGitHubForm.repoUrl.trim(),
        branch: importGitHubForm.branch.trim() || undefined,
        path: importGitHubForm.path.trim() || undefined,
        pat: importGitHubForm.pat.trim() || undefined
      })
    } else {
      if (!azureInventory.value) throw new Error('Load the inventory first.')
      const apiNames = azureInventory.value.apis
        .filter((api: any) => api.selected)
        .map((api: any) => api.name)
      imported = await importService.azureImport({ ...importAzureForm, apiNames })
    }
...
function hydrateFromProject(imported: any) {
  projectName.value = imported.name || projectName.value
  projectDescription.value = imported.description || projectDescription.value
  globalPolicy.value = imported.globalPolicy || ''

  project.apis.splice(0, project.apis.length, ...(imported.apis ?? []).map((api: any) => ({
    name: api.name,
    displayName: api.displayName || api.name,
    path: api.path || '',
    description: api.description || '',
    protocols: api.protocols?.length ? api.protocols : ['https'],
    serviceUrl: api.serviceUrl || '',
    subscriptionKeyParameterName: api.subscriptionKeyParameterName || 'Ocp-Apim-Subscription-Key',
    apiVersion: api.apiVersion || '',
    apiVersionSetId: api.apiVersionSetId || '',
    apiRevision: api.apiRevision || '1',
    isCurrent: api.isCurrent ?? true,
    subscriptionRequired: api.subscriptionRequired ?? true,
    policy: api.policy || defaultApiPolicy(),
    operations: (api.operations ?? []).map((op: any) => ({
      name: op.name,
    ...
    specificationContent: api.specificationContent || '',
    specificationFormat: api.specificationFormat || 'openapi'
  })))

  project.backends.splice(0, project.backends.length, ...(imported.backends ?? []).map((backend: any) => ({
    name: backend.name,
  ...
  project.namedValues.splice(0, project.namedValues.length, ...(imported.namedValues ?? []).map((namedValue: any) => ({
    name: namedValue.name,
    displayName: namedValue.displayName || namedValue.name,
    value: namedValue.value || '',
    secret: namedValue.secret ?? false,
    tags: namedValue.tags ?? []
  })))

  project.policyFragments.splice(0, project.policyFragments.length, ...(imported.policyFragments ?? []).map((fragment: any) => ({
    name: fragment.name,
    description: fragment.description || '',
    policyContent: fragment.policyContent || ''
  })))

  project.products.splice(0, project.products.length, ...(imported.products ?? []).map((product: any) => ({
    name: product.name,
    displayName: product.displayName || product.name,
    description: product.description || '',
    subscriptionRequired: product.subscriptionRequired ?? true,
    approvalRequired: product.approvalRequired ?? false,
    published: product.published ?? true,
    apis: product.apis ?? []
  })))

  project.schemas.splice(0, project.schemas.length, ...(imported.schemas ?? []))
...
function openAddModal(kind: ArtifactModalKind) {
  ...
  if (kind === 'api') {
    ...
    libraryService.getProducts()
      .then(items => { libraryProducts.value = items })
      .catch(() => { libraryProducts.value = [] })
  } else if (kind === 'namedValue') {
    Object.assign(namedValueDraft, createNamedValueDraft())
    libraryService.getNamedValues()
      .then(items => { libraryNamedValues.value = items })
      .catch(() => { libraryNamedValues.value = [] })
  } else {
    Object.assign(fragmentDraft, createPolicyFragmentDraft())
    fragmentService.getFragments()
      .then(items => { libraryFragments.value = items })
      .catch(() => { libraryFragments.value = [] })
    templateService.getTemplates()
      .then(items => { policyTemplates.value = items })
      .catch(() => { policyTemplates.value = [] })
  }
}

function applyProductFromLibrary(name: string) {
  const item = libraryProducts.value.find(product => product.name === name)
  if (!item) return
...
function applyNamedValueFromLibrary(name: string) {
  const item = libraryNamedValues.value.find(namedValue => namedValue.name === name)
  if (!item) return
  Object.assign(namedValueDraft, {
    name: item.name,
    displayName: item.displayName || item.name,
    value: item.value || '',
...
function applyFragmentStart(key: string) {
  if (key.startsWith('fragment:')) {
    const item = libraryFragments.value.find(fragment => `fragment:${fragment.id}` === key)
    if (!item) return
    fragmentDraft.name = item.name
    fragmentDraft.description = item.description || ''
    fragmentDraft.policyContent = item.xml
  } else if (key.startsWith('template:')) {
    const item = policyTemplates.value.find(template => `template:${template.id}` === key)
    if (!item) return
    fragmentDraft.name = `${item.id}-fragment`
    fragmentDraft.description = `Fragment wrapping the "${item.name}" template`
    fragmentDraft.policyContent = wrapTemplateInFragment(item.xml)
  }
}

function wrapTemplateInFragment(templateXml: string) {
  const indented = templateXml
    .split('\n')
...
function saveArtifact(closeAfterSave: boolean) {
  if (artifactModalKind.value === 'api') {
    ...
    if (saveToLibrary.value) {
      libraryService.saveProduct(product).catch(error => console.error('Failed to save product to library:', error))
    }
    ...
    if (saveToLibrary.value) {
      libraryService.saveNamedValue(namedValue).catch(error => console.error('Failed to save named value to library:', error))
    }
    ...
    if (saveToLibrary.value) {
      fragmentService.createFragment({
        name: fragment.name,
        description: fragment.description,
        xml: fragment.policyContent,
        version: 1
      }).catch(error => console.error('Failed to save fragment to library:', error))
    }
...
function saveDraft() {
  ...
  try {
    const draft = {
      ...
      globalPolicy: globalPolicy.value,
      project
    }
...
function restoreDraft() {
  ...
  try {
    ...
    globalPolicy.value = draft.globalPolicy || ''
...
async function generateProject() {
  ...
  const projectData = {
    ...
    globalPolicy: globalPolicy.value || undefined,
    ...project
  }
